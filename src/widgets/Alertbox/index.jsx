          ...
          <div className={style.wrap}>
            <div className={style.alertImageWrap}>
              <div
                ...
                style={{
                  backgroundImage: `url("https://isetup.vn/tiktok/assets/gif/Explosion.gif")`,
                }}
              >
                <img
                  style={{ height: "1px", opacity: "0", width: "1px" }}
                  src="https://isetup.vn/tiktok/assets/gif/jumpy-t-rex.gif"
                  alt="animate gif"
                />
              </div>
            </div>
...
const TextAnimation = (props) => {
  ...
  const template =
    settings[`widget_alertbox_${event.name}_messagetemplate`] ||
    defaultTemplate[event.name]
  ...
  const animateText = (string) => {
    ...
    return string.split(" ").map((text, index2) => {
      ...
      return (
        <span key={index2} className="text-nowrap">
          {" "}
          {arr.map((letter, index1) => (
            <span
              style={{ color: color }}
              key={index1}
              className={"animated-letter " + animateName}
            >
              {letter}
            </span>
          ))}{" "}
  ...
  return template
    ...
    .map((text, index) => (
      <span key={index}>
        {text === "{username}"
          ? animateText(event.uniqueId)
          : text === "{nickname}"
          ? animateText(event.nickname || event.uniqueId)
          : text === "{giftname}"
          ? animateText(event.giftName)
          : text === "{giftcount}"
          ? animateText(event.repeatCount)
          : text === "{likecount}"
          ? animateText(event.likeCount)
          : text === "{amount}"
          ? animateText(event.repeatCount * event.diamondCount)
          : text}{" "}
      </span>
    ))
}
...
const Alertbox = (props) => {
  ...
  useEffect(() => {
    return () => {
      const audio = new Audio(
        "https://isetup.vn/tiktok/assets/sound/new-message-4.ogg"
      )
      audio.volume = 0
  ...
  useEffect(
    function () {
      ...
      setIsOutro(false)
      const introAnimate = !_isShowing
        ? getSetting("alertanimationin", event.name)
        : getSetting("alertparries")
        ? "fadeIn"
        : getSetting("alertanimationin", event.name)
      ...
      playSound(
        getSetting("soundurl", event.name),
        getSetting("soundvolume", event.name)
      )
      ...
      setTimeout(() => {
        if (document.getElementById(event.id)) {
          ...
          const outroAnimate =
            events.length === 0
              ? getSetting("alertanimationout", event.name)
              : getSetting("alertparries")
              ? "fadeOut"
              : getSetting("alertanimationout", event.name)
  ...
  return (
    <div className="Alertbox">
      ...
      <div
        className={`${style.alertboxLayer} ${
          style[getSetting("layout", mainEvent.name || "like")]
        }`}
      >
        {_isShowing && mainEvent.id && (
          <div
            className={`${style.widget} ${animate}`}
              ...
              <div className={style.wrap}>
                <div className={style.alertImageWrap}>
                  ...
                  <div
                    className={style.alertImage}
                    style={{
                      backgroundImage: `url(${getSetting(
                        "imageurl",
                        mainEvent.name
                      )})`,
                    }}
                  >
                    <img
                      style={{ height: "1px", opacity: "0", width: "1px" }}
                      src={getSetting("imageurl", mainEvent.name)}
                      alt="animate gif"
                    />
                  </div>
                </div>
                  ...
                  <div className={style.alertText}>
                    <div
                      style={{
                        ...
                        color: `${
                          getSetting("textcolor", mainEvent.name) ||
                          "rgb(255, 255, 255)"
                        }`,
                        fontFamily:
                          "-apple-system, BlinkMacSystemFont, 'Segoe UI', 'Roboto', 'Oxygen', 'Ubuntu', 'Cantarell', 'Fira Sans', 'Droid Sans', 'Helvetica Neue'",
                        fontWeight: `${getSetting(
                          "fontweight",
                          mainEvent.name
                        )}`,
                        textShadow:
